...
# This file was taken from HOMER and modified to incorporate
# strain specific sequences
# HOMER is available at homer.ucsd.edu/homer
# The author of HOMER is Christopher Benner (cbenner@ucsd.edu)
#
...
sub printCMD {
	...
	print STDERR "\t\t-data_dir <dir>: directory with MMARGE files for strains (default defined in config)\n";
...
if ($cmd->{'bg'} eq '') {
	...
	print STDERR "remove overlapping target and background positions\n";
	`mergePeaks "$cleanBgFile" "$cleanPosFile" -d $cmd->{'size'} -cobound 1 -prefix "$tmpFile2"`;
	print STDERR "mv " . $tmpFile2 . ".coBoundBy0.txt " . $cleanBgFile . "\n";
