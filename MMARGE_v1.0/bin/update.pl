...
$_ = () for my(@split, @MMARGE_folders, @motif_bg, @motif_analysis_folders, %save_output, @tmp, $list);
$_ = 0 for my($MMARGE_folders_list, $motif_bg_list, $motif_window, $motif_analysis_list, $software_update);
$_ = "" for my($tmp_output, $tmp_error, $command, $MMARGE_genome, $file);

my $url = "http://homer.ucsd.edu/MMARGE/";
 
sub printCMD{
	print STDERR "Script to download already pre-processed data for MMARGE\n";
	print STDERR "The script offers downloads for MMARGE folders and individualzed genomes, as well as background scans for motifs and pre-processed files for motif analysis per strain (200bp)\n";
	print STDERR "\n\n";
	print STDERR "Usage:\n";
	print STDERR "-genome <genome>: The genome for which the motifs were scanned for (e.g. mm10/hg19)\n";
	print STDERR "-MMARGE_folders <list of folders>: Downloads the list of all folders specified in the list. If all: downloads all available folders\n";
	print STDERR "-MMARGE_folders_list: Lists all MMARGE folders that are available for downloadn\n";
	print STDERR "-motif_bg <list of motifs>: Downloads all the motif background scans for the motifs listed here. If all: downloads all available motifs\n";
...
GetOptions(	"MMARGE_folders=s{,}" => \@MMARGE_folders,
		"MMARGE_folders_list" => \$MMARGE_folders_list,
		"genome=s" => \$MMARGE_genome,
		"motif_bg=s{,}" => \@motif_bg,
...
if($MMARGE_folders_list == 1) {
	&list_MMARGE_folders(1);
	exit;
}
if($motif_bg_list == 1) {
	&list_motif_bg(1);
...
if((@MMARGE_folders >= 1 || @motif_bg >= 1 || @motif_analysis_folders >= 1) && $MMARGE_genome eq "") {
	print STDERR "Please specify genome!\n\n";
	exit;
}

if(@MMARGE_folders == 1) {
	@tmp = split(",", $MMARGE_folders[0]);
	if(@tmp > @MMARGE_folders) {
		@MMARGE_folders = @tmp;
	}
}

for(my $i = 0; $i < @MMARGE_folders; $i++) {
	$MMARGE_folders[$i] =~ s/,//g;
	$MMARGE_folders[$i] = uc($MMARGE_folders[$i]);
}

if(@MMARGE_folders >= 1) {
	$list = &list_MMARGE_folders(0);
	if(@MMARGE_folders == 1 && uc($MMARGE_folders[0]) eq "ALL") {
		@MMARGE_folders = ();
		foreach my $s (keys %{$list->{$MMARGE_genome}}) {
			push @MMARGE_folders, $s;
		}
	}
	for(my $i = 0; $i < @MMARGE_folders; $i++) {
		if(!exists $list->{$MMARGE_genome}->{$MMARGE_folders[$i]}) {
			print STDERR "Could not find " . $MMARGE_folders[$i] . " for " . $MMARGE_genome . "\n";
		} else {
			&download_file_MMARGE($MMARGE_genome, $MMARGE_folders[$i]);
		}
	}
}
...
if(@motif_analysis_folders >= 1) {
	...
	if(@motif_analysis_folders == 1 && uc($motif_analysis_folders[0]) eq "ALL") {
		...
		foreach my $s (keys %{$list->{$MMARGE_genome}->{$motif_window}}) {
			push @motif_analysis_folders, $s;
		}
	}
	for(my $i = 0; $i < @motif_analysis_folders; $i++) {
		if(!exists $list->{$MMARGE_genome}->{$motif_window}->{$motif_analysis_folders[$i]}) {
			print STDERR "Could not find " . $motif_analysis_folders[$i] . " for " . $MMARGE_genome . " with window " . $motif_window . "\n";
			print STDERR "Run this script with parameter -motif_analysis_list to see what is available\n";  
		} else {
			&download_motif_analysis($MMARGE_genome, $motif_analysis_folders[$i], $motif_window);
		}
	}
}
...
sub download_file_MMARGE{
	my $g = $_[0];
	my $f = $_[1];
	print STDERR "Downloading " . $f . "\n";
	$command = "wget " . $url . "/MMARGE_folders/" . $g . "_" . $f . ".tar.gz";
	`$command`;
	#Unzip file
...
sub download_motif_analysis{
	...
	if(!-e $config->{'data_folder'} . "/" . $f) {
		...
		print STDERR "Please download first the MMARGE folder for this individual genome!\n";
		exit;
	}
...
sub list_MMARGE_folders {
	my $print = $_[0];
	$tmp_output = "output_" . rand(5) . ".txt";
	$tmp_error = "error_" . rand(5) . ".txt";
	`wget -O $tmp_output $url/MMARGE_folders/list.txt 2> $tmp_error`;
	open FH, "<$tmp_output";
	foreach my $line (<FH>) {
		chomp $line;
