...
for(my $i = 0; $i < @strains; $i++) {
	...
	for(my $a = 0; $a < $allele; $a++) {
		$sth = $dbh->prepare_cached("SELECT * FROM " . $genome . "_mutations_" . $strains[$i] . "_allele_" . ($a + 1) . " WHERE chr = ? AND pos >= ? AND pos <= ?");
		$handles[$i] = $sth;
	}
}
	...
	if(@split == 0) {
		...
		if($exons == 1) {
			...
			while(my $e = $sth_exon->fetchrow_hashref()) {
				...
				for(my $i = 0; $i < @strains; $i++) {
					for(my $a = 0; $a < $allele; $a++) {
						$handles[$i][$a]->execute($chr, $e->{'start'}, $e->{'stop'});
						while(my $mut = $handles[$i][$a]->fetchrow_hashref()) {
			...
			for(my $i = 0; $i < @strains; $i++) {
					if($strains[$i] eq "reference") { next; }
					for(my $a = 0; $a < $allele; $a++) {
						$handles[$i]->execute($chr, $start, $end);
						while(my $mut = $handles[$i]->fetchrow_hashref()) {
							$mutations .= $mut->{'pos'} . ":" . $mut->{'reference'} . "->" . $mut->{'strain'} . ";";
						}
						$mutations .= "\t";
					}
