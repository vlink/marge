...
$_ = 0 for my($hetero, $bp, $resize, $allele, $mut_exists, $min, $max, $center, $first, $no_resize, $no_ann, $F1);
...
sub printCMD {
	...
	print STDERR "\t-F1: Annotate allele-specific based on a F1 mouse strain. In this case, please specify only 2 genomes - both parental mouse strains. These genomes are then used as different alleles. This does only work for two genoems, not more!\n";
	print STDERR "\n\n\n";
	exit;
}
...
if($no_ann == 0) {
	...
	if(@ann_perfect == 0) {
		for(my $i = 0; $i < @tag_perfect; $i++) {
			$command = "annotatePeaks.pl " . $merged_file . " none -d " . $tag_perfect[$i] . " -noadj > merged_peaks_ann_perfect_" . $tag_perfect_name[$i] . ".txt";
	...
	if(@ann_mut == 0) {
		for(my $i = 0; $i < @tag_mut; $i++) {
			$command = "annotatePeaks.pl " . $merged_file . " none -d " . $tag_mut[$i] . " -noadj > merged_peaks_ann_muts_" . $tag_mut_name[$i] . ".txt";
			print STDERR $command . "\n";
			`$command`;
		}
	}
