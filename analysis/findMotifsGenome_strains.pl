...
sub printCMD {
	...
	print STDERR "\t\t-fg_ind <strain>: Individual used in the foreground\n";
	print STDERR "\t\t-bg_strain <strain>: Strain used in the background\n";
	print STDERR "\t\t-bg_ind <strain>: Individual used in the background\n";
...
if(!exists $cmd->{'fg_strain'}) {
	print STDERR "Foreground strain has to be defined\n";
	print STDERR "If background strain is not defined, the foreground strain is used\n";
	exit;
}
if(!exists $cmd->{'bg_strain'}) {
	$cmd->{'bg_strain'} = $cmd->{'fg_strain'};
}
...
if ($cmd->{'bg'} eq '') {
	...
	#First check if strain preparsed file exists
	if(-e $preparsedDir_strain) {
		if(glob($preparsedDir_strain . "/" . $cmd->{'bg_strain'} . ".*cgbins")) {
			$preparsedDir = $preparsedDir_strain;
			$prefix = $cmd->{'bg_strain'};
			$preparse_strain_exists = 1;
		}
	}
	...
	if ($preparseFound == 1 && $cmd->{'preparseForce'} == 0) {
		...
		if($preparse_strain_exists == 0) {
			print STDERR "\tGenerating strain background\n";
			&generate_strain_bg();
		} else {
			print STDERR "\tBackground files for " . $cmd->{'bg_strain'} . " for $bestFragSize bp fragments found.\n";
		}	
	} else {
...
if ($executeFlag==1) {
	if ($cmd->{'refFlag'} == 1) {
		...
		`perl /gpfs/data01/glasslab/home/vlink/mouse_strains/marge/analysis/extract_seq_from_peakfiles.pl -file "$posFileSize" -ind "$cmd->{'fg_strain'}" -genome_dir "$genome_dir" -output "$tmpFile" -data_dir "$data_dir" -id`;	
		print "perl /gpfs/data01/glasslab/home/vlink/mouse_strains/marge/analysis/extract_seq_from_peakfiles.pl -file " . $posFileSize . " -ind " . $cmd->{'fg_strain'} . " -genome_dir " . $genome_dir . " -output " . $tmpFile . " -data_dir " . $data_dir . " -id\n";	
		#End of strain code
		####################
...
if ($cmd->{'find'} eq '') {
	...
	if ($executeFlag == 1) {	
		...
		my $col = 1;
		$col = 2 if ($cmd->{'gc'} == 1);
		print STDERR "Here we do calculate targetCGBins\n";
...
sub generate_strain_bg {
	...
	$command = "perl /gpfs/data01/glasslab/home/vlink/mouse_strains/marge/analysis/extract_seq_from_peakfiles.pl -file " . $posStrain . " -ind " . $cmd->{'bg_strain'} . " -genome_dir " . $genome_dir . " -output " . $seqStrain . " -data_dir " . $data_dir . " -id";
	#print $command . "\n";
	`$command`;
	#Calcualte GC frequency
	$command = "homerTools freq -gc " . $gcfreqStrain . " " . $seqStrain . " > tmp.txt";
	`$command`;
	$command = "freq2group.pl " . $gcfreqStrain . " > " . $cgBinsStrain;
	`$command`;
 	$command = "freq2group.pl " . $gcfreqStrain . " 2 > " . $gcBinsStrain;
	`$command`;
	`rm tmp.txt`;
	$preparsedDir = $preparsedDir_strain;
	$prefix = $cmd->{'bg_strain'};
