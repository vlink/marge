...
sub printCMD{
	...
	print STDERR "\t-complete <list of individuals/strains>: Downloads all data per strain (MARGE folder, motif background, STAR index and bowtie index>\n\t\tIf set to all: downloads all available folders (NOT RECOMMENDED!)\n\t\tIf empty: Lists all available MARGE folders\n";
...
if(@complete >= 1) {
	...
	for(my $i = 0; $i < @MARGE_folders; $i++) {
		if(!exists $list_marge_folders->{$MARGE_genome}->{$MARGE_folders[$i]}) {
			...
			print STDERR "Run this script without specifying any indices or with -MARGE_folders_list to see a list of all available indices\n";
		} else {
