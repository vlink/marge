...
sub read_config{
	open CONFIG, "</data/home/vlink/mouse_strains/software/config/config.txt";
	my %parameters;
	my @split;
...
sub chromosome_number{
	my %chr;
	open CHR, "</data/home/vlink/mouse_strains/software/config/chromosomes.txt";
	foreach my $line (<CHR>) {
		chomp $line;
		if($line eq "") { next; }
		@split = split('\t', $line);
...
sub gene_format{
	open GENE, "</data/home/vlink/mouse_strains/software/config/formats.txt" or die ("formats.txt does not exist!\n");;
	my %gf;
	my @s;
...
sub name_format{
	open NAME, "</data/home/vlink/mouse_strains/software/config/formats.txt" or die("formats.txt does not exist!\n");
	my %nf;
	my $name;
	my $run = 0;
	my @s;
