...
sub read_strains_mut {
	...
	foreach my $file (@mut_files) {
		...
		open FH, "<$file";
		my $tree = Set::IntervalTree->new;
		my $tree_detail = Set::IntervalTree->new;
		foreach my $line (<FH>) {
			chomp $line;
			@split = split('\t', $line);
			$tree->insert( {'mut'=> $split[0] }, $split[0], $split[0] + length($split[2]));
			$tree_detail->insert( {'pos'=> $split[1] . "->" . $split[2]}, $split[0], $split[0] + length($split[2]));	
		}
		close FH;
		$tree_mut{$chr}{$allele} = $tree;
		$tree_detail{$chr}{$allele} = $tree_detail;
	}
	$file = $data_dir . "/" . $strain . "/last_shift_ref.txt";
	if(-e $file) {
		%last = %{retrieve($file)};
	}
	return (\%tree_mut, \%tree_detail, \%last);
}
