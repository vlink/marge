...
my (@files, @strains, $dbh, $sth, @split, $c, $shift, %shift_vector, $allele, @a);
my $print = 1;
$allele = 1;
...
if(@strains == 1) {
	&shift_file(\@files, $strains[0]);
	@files= ();
}

#Remove commas
for(my $i = 0; $i < @files; $i++) {
	print STDERR "What the hell are we doing here?\n";
	chomp $files[$i];
	$files[$i] =~ s/,//g;
}
...
if($core == 1) {
	for(my $i = 0; $i < @files; $i++) {
		print STDERR "What the hell are we doing here core?\n";
		$a[0] = $files[$i];
		&shift_file(\@a, $strains[$i]);
	}	
} else {
	...
	my $run = 0;
		...
		print STDERR "What the hell are we doing here threading?\n";
                @running = threads->list(threads::running);
                if (scalar @running < $core) {
			$a[0] = $files[$run];
                        my $thread = threads->new( sub { shift_file(\@a, $strains[$run]) });
			$run++;
                        push (@Threads, $thread);
                        my $tid = $thread->tid;
                }
...
sub shift_file{
	...
	foreach my $c (keys %{$total_chr}) {
		...
		for(my $k = 0; $k < @f; $k++) {
			chomp $f[$k];
			$name = $f[$k] . "_shifted_from_" . $s;
			open OUT, ">>$name";
			print STDERR "Shifting $f[$k] from $s to reference!\n";
				...
				for(my $j = 0; $j < @{$shift_order}; $j++) {
					if($shift_order->[$j] eq "shift" ) {
						if($split[$j] < $start) {
							...
							if(!exists $shift_vector{$split[$j]}) {
								print STDERR $line . "\n";
								print STDERR $split[$j] . "\n";
								exit;
							}
